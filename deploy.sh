...
echo "Installing Java 8 on all instances..."

for dns in $dnslist; do
    ssh -i AutoDeployKP.pem ec2-user@$dns 'sudo yum install java-1.8.0-openjdk -y; sudo service docker start'
done

echo "Installing maven on all instances..."

for dns in $dnslist; do
    ssh -i AutoDeployKP.pem ec2-user@$dns 'sudo yum install maven -y; sudo service docker start'
done
...
for dns in $dnslist; do
    ssh -i AutoDeployKP.pem ec2-user@$dns 'cd ~/intro-workshop-aygo/roundrobin/; mvn clean install'
    ssh -i AutoDeployKP.pem ec2-user@$dns 'cd ~/intro-workshop-aygo/logservice/; mvn clean install'
done
